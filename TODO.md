...
- [x] **Fix `spatial_thinning()` target method** — last commit notes it doesn't work. Debug the call chain from `spatial_thinning()` → `validate_arg_target()` → `thinning_to_target()` (C++).
- [x] **Commit untracked test files** — `tests/testthat/test-validate_arg_distance.R` and `test-validate_arg_target.R` are untracked (`??` in git status).
- [x] **Fix binary search tolerance in `method_contiguous_planar.cpp`** — fixed absolute tolerance `0.01` breaks for projected CRS (meters). Replace with relative tolerance (e.g., `1e-4 * scale`).
